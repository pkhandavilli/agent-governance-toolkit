...
package agt.examples.templates.healthcare
...
candidates contains {"name": "healthcare-asi01-cbrn-guardrail", "action": "deny", "priority": 100, "message": "ASI-01: Goal Hijack — detected attempt to solicit high-risk synthesis via persona framing"} if {
	regex.match(`(?i)((synthesis|recipe|formula|procedure|precursor|chemical).* (Mustard\s+Gas|Sarin|VX|Cyanide|Anthrax|Pathogen|Toxin)|(academic|educational|hypothetical|research|fictional)\s+inquiry\s+into\s+(harmful|lethal|toxic|chemical)\s+agents)`, sprintf("%v", [object.get(context, "output", 0)]))
}
...
candidates contains {"name": "asi02-block-file-deletion", "action": "deny", "priority": 95, "message": "ASI-02: Destructive file/data operations are prohibited"} if {
...
candidates contains {"name": "asi08-session-tool-call-limit", "action": "deny", "priority": 85, "message": "ASI-08: Session tool call limit exceeded — circuit breaker engaged"} if {
	to_number(object.get(context, "tool_call_count", 0)) > 25
}
...
candidates contains {"name": "healthcare-block-phi-mrn", "action": "deny", "priority": 100, "message": "ASI-01/06: HIPAA: Medical Record Number pattern detected in output"} if {
	regex.match(`(?i)MRN[:\s]*\d{6,}`, sprintf("%v", [object.get(context, "output", 0)]))
}

candidates contains {"name": "healthcare-block-phi-dea", "action": "deny", "priority": 100, "message": "ASI-01/06: HIPAA: DEA Number pattern detected in output"} if {
	regex.match(`(?i)DEA[:\s]*[A-Z]{2}\d{7}`, sprintf("%v", [object.get(context, "output", 0)]))
}

candidates contains {"name": "healthcare-audit-phi-npi", "action": "warn", "priority": 80, "message": "ASI-01/06: HIPAA: Possible NPI (10-digit number) detected. Subject to 6-year HIPAA audit retention."} if {
	regex.match(`\b\d{10}\b`, sprintf("%v", [object.get(context, "output", 0)]))
}

candidates contains {"name": "healthcare-audit-diagnosis-codes", "action": "warn", "priority": 75, "message": "ASI-01/06: HIPAA: Clinical coding pattern detected. Subject to 6-year HIPAA audit retention."} if {
	regex.match(`(?i)(ICD-10[:\s]*[A-Z]\d{2}|CPT[:\s]*\d{5})`, sprintf("%v", [object.get(context, "output", 0)]))
}

candidates contains {"name": "healthcare-block-diagnosis-codes", "action": "warn", "priority": 50, "message": "ASI-01/06: HIPAA: Diagnosis/Procedure code audit-logged for medical billing compliance."} if {
	regex.match(`(?i)(ICD-10|ICD-11|CPT-4|SNOMED)`, sprintf("%v", [object.get(context, "output", 0)]))
}
...
candidates contains {"name": "healthcare-enforce-deidentification", "action": "deny", "priority": 99, "message": "ASI-02/06: HIPAA: Direct export blocked. You must use the 'mask_phi' or 'de_identify_data' tool before external transfer."} if {
	regex.match(`^(export_report|email_patient|send_message|external_api_call)$`, sprintf("%v", [object.get(context, "action", 0)]))
}

candidates contains {"name": "healthcare-minimum-necessary-read", "action": "allow", "priority": 55, "message": "ASI-01/06: HIPAA: Minimum-necessary enforcement for safe reads."} if {
	regex.match(`^(read_summary|lookup_patient)$`, sprintf("%v", [object.get(context, "action", 0)]))
}

candidates contains {"name": "healthcare-prevent-bulk-exposure", "action": "deny", "priority": 95, "message": "ASI-01/06: HIPAA: Bulk PHI exposure blocked (minimum-necessary enforcement)."} if {
	regex.match(`(?i)(patient_id|mrn).*(\n.*){50,}`, sprintf("%v", [object.get(context, "output", 0)]))
}

candidates contains {"name": "healthcare-allow-read-operations", "action": "allow", "priority": 50, "message": ""} if {
	regex.match(`^(read_|search_|lookup_|list_|get_)`, sprintf("%v", [object.get(context, "action", 0)]))
}

candidates contains {"name": "healthcare-allow-summarize", "action": "allow", "priority": 50, "message": ""} if {
	object.get(context, "action", 0) == "summarize"
}
