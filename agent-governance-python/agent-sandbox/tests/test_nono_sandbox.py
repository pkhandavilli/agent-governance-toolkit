...
class _FakeProxy:
    ...
    def sandbox_env(self, extra: list[tuple[str, str]] | None=None) -> list[tuple[str, str]]:
...
class _FakeProxyConfig:
    ...
    def __init__(self, allowed_hosts: list[str] | None=None, allow_all_hosts: bool=False) -> None:
...
class _FakeNono:
    ...
    def sandboxed_exec(self, caps: _FakeCaps, command: list[str], cwd: str | None=None, timeout_secs: float | None=None, env: list[tuple[str, str]] | None=None, inherit_env: bool=False) -> _FakeExecResult:
        self.last_caps = caps
...
class TestNonoConfig:
    ...
    def test_from_sandbox_config(self):
        sc = SandboxConfig(timeout_seconds=10, network_enabled=True, network_default='allow', input_dir='/in', output_dir='/out', env_vars={'K': 'V'})
    ...
    def test_sanitized_env_strips_loader_hooks(self):
        cfg = NonoConfig(env_vars={'SAFE': 'ok', 'LD_PRELOAD': '/tmp/evil.so'})
...
class TestSandboxConfigTranslation:
    ...
    def test_mounts_and_egress(self):
        cfg = NonoConfig.from_sandbox_config(
            SandboxConfig(
                timeout_seconds=45,
                input_dir='/data/in',
                output_dir='/data/out',
    ...
    def test_protected_mount_rejected(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match='protected system directory'):
            NonoConfig.from_sandbox_config(SandboxConfig(input_dir='/usr'))
...
class TestLifecycle:
    ...
    def test_create_execute_destroy(self, provider, fake_nono):
        ...
        assert provider.get_session_status(handle.agent_id, handle.session_id) == SessionStatus.READY
        execution = provider.execute_code(handle.agent_id, handle.session_id, "print('hi')")
        ...
        assert provider.get_session_status(handle.agent_id, handle.session_id) == SessionStatus.DESTROYED
    ...
    def test_nonzero_exit_is_failure(self, provider, fake_nono):
        ...
        handle = provider.create_session('agent-1')
        execution = provider.execute_code(handle.agent_id, handle.session_id, 'print(1)')
    ...
    def test_run_once_destroys_session_on_guard_violation(self, provider, fake_nono):
        with pytest.raises(SandboxCodeViolation):
            provider.run_once('agent-1', "import subprocess; subprocess.run(['ls'])")
        assert provider._sessions == {}
...
class TestCapabilityWiring:
    ...
    def test_env_not_inherited_and_context_injected(self, provider, fake_nono):
        handle = provider.create_session('agent-1')
        provider.execute_code(handle.agent_id, handle.session_id, 'print(1)', context={'task': 'demo'})
        assert fake_nono.last_inherit_env is False
...
class TestSpawn:
    ...
    def test_output_truncated(self, provider, fake_nono):
        fake_nono.stdout = b'x' * 2000000
        handle = provider.create_session('agent-1')
        execution = provider.execute_code(handle.agent_id, handle.session_id, 'print(1)')
        assert 'truncated' in execution.result.stdout

    def test_timeout_passed_through(self, provider, fake_nono):
        handle = provider.create_session('agent-1', config=SandboxConfig(timeout_seconds=12))
        provider.execute_code(handle.agent_id, handle.session_id, 'print(1)')
...
class TestRawRun:
    ...
    def test_run_with_session(self, provider, fake_nono):
        ...
        handle = provider.create_session('agent-1')
        result = provider.run(handle.agent_id, ['echo', 'hi'], session_id=handle.session_id)
...
class TestGuards:
    ...
    def test_code_scanner_blocks_subprocess(self, provider, fake_nono):
        handle = provider.create_session('agent-1')
        with pytest.raises(SandboxCodeViolation):
            provider.execute_code(handle.agent_id, handle.session_id, "import subprocess; subprocess.run(['ls'])")
    ...
    def test_policy_deny_blocks_execution(self, provider, fake_nono):
        ...
        class _DenyRuntime:
            ...
            def close(self):
                return None
        handle = provider.create_session('agent-1', runtime=_DenyRuntime())
