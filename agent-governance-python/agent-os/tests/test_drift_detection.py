...
class TestComputeDrift:
    ...
    @staticmethod
    def _ctx(**settings):
        integration = BaseIntegration(runtime=_Runtime(), **settings)
        return integration.create_context("agent-drift")
    ...
    def test_drift_result_repr(self):
        dr = DriftResult(score=0.5, exceeded=True, threshold=0.15, baseline_hash='aaa', current_hash='bbb')
        assert 'EXCEEDED' in repr(dr)
        dr2 = DriftResult(score=0.1, exceeded=False, threshold=0.15, baseline_hash='aaa', current_hash='bbb')
...
class TestPostExecuteDrift:
    ...
    @staticmethod
    def _kernel(**settings):
        return BaseIntegration(runtime=_Runtime(), **settings)
    ...
    def test_drift_disabled_when_threshold_zero(self):
        ...
        assert valid is True
        assert ctx._baseline_hash is None
