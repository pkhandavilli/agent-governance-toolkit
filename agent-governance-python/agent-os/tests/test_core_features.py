...
class TestAgentConfigSerialization:
    ...
    def test_round_trip(self) -> None:
        config = AgentConfig(agent_id='ser-agent', policies=['read_only'], metadata={'team': 'core'}, max_audit_log_size=500)
        d = config.to_dict()
...
class TestAuditEntrySerialization:
    ...
    def test_round_trip(self) -> None:
        entry = AuditEntry(timestamp=datetime.now(timezone.utc), agent_id='audit-agent', request_id='req-1', action='read', params={'file': 'a.txt'}, decision=PolicyDecision.ALLOW, result_success=True, execution_time_ms=12.5)
        d = entry.to_dict()
    ...
    def test_from_dict_with_none_fields(self) -> None:
        d = {'timestamp': '2025-01-01T00:00:00+00:00', 'agent_id': 'a1', 'request_id': 'r1', 'action': 'write', 'params_keys': ['x'], 'decision': 'deny'}
